  ...
  # Vinicius Piccoli  - PhD student at M3G - UNICAMP
 
  #= Informations:

  This simple code reads protein pdb files and writes inputs for Packmol.

  The code only works for a simulation of a protein solvated by water and ionic liquids (e.g EMIMDCA, EMIMBF4, BMIMNO3, BMIMCl, etc.)
   
  Example:
  gmx pdb2gmx -f protein.pdb (crystallographic structure) -o processed.pdb 
  measure_prot(processed.pdb,10) - this will return the dimensions of the protein in x,y and z axis plus 10 Angstrons.   
   
  The conc function write an “inp” file (which will be the input for Packmol and, therefore, contains the information for the simulation box) 
  and the topology file for the simulation
  
  conc(MM::Float64,Nome::String,Nomean::String, conc::Float64)
    MM - Ionic Liquid molar mass
    Nome - Name of the cation
    Nomean - Name of the anion
    conc - Ionic liquid concentration
  =#


  function measure_prot(nome,dist)
  ...
  ## Box size - add 30 A in each size of box
    bx = round(Int,((xmax - xmin) + dist )) 
    by = round(Int,((ymax - ymin) + dist ))
    bz = round(Int,((zmax - zmin) + dist ))
  ...
  function conc(MM::Float64,Nome::String,Nomean::String, conc::Float64)
      ...
      # Box dimensions
      lx,ly,lz = measure_prot("ubq.pdb")   # Ubiquitin pdb
      
            
      # Total concentration of ions
      c = conc;
  
      vs   = vol_box(2*lx,2*ly,2*lz) - vol_prot(8560); # Solution volume / Ubiquitin molar mass = 8560 
      nil  = round(Int,(num_il(vs,c)));                # Number of ionic liquid molecules for a concentration "c"  
   
      nwat1 = num_wat(vs, v_il(nil,MM));               # Number of water molecules 
      nwat = nwat1 
      ...
      for line in eachline(topa) 
        ...
        elseif ntline == (nlines - 3) 
          println(top,replace(line,line => "$(Nome[1:3])                $nil ",count = 1))          
        else
          println(top,line)
        end
      end
