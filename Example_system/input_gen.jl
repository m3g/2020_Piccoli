  ...
  ## Box size - add 30 A in each size of box
    bx = round(Int,((xmax - xmin) + dist + dist)/2) 
    by = round(Int,((ymax - ymin) + dist + dist)/2)
    bz = round(Int,((zmax - zmin) + dist + dist)/2)
  ...
  function conc(MM::Float64,Nome::String,Nomean::String, conc::Float64,;dist=30)
      ...
      # Box dimensions
      lx,ly,lz = measure_prot("ubq.pdb",dist) 
      ...
      nwat = num_wat(vs, v_il(nil,MM));               # Number of water molecules 
